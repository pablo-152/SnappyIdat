...
<!--
<?php
    $query_c=mysqli_query($con, "SELECT * FROM conversatorio WHERE id_conversatorio IN (8,9,10)");
            ...
            <div class="item-1">
                <div class="subitem-1">
                    <div class="subitemdos-2">
                        <h5 style="display: flex;justify-content: center;align-items: center;"><label class="linea1">&nbsp;Conversatorio&nbsp;</label></h5>
                        <h1 class="linea2">Educacion y presencialidad:</h1>
                        <h1 class="linea3">Mitos y Experiencias</h1>
    ...
    function Insert_Inscripcion(){
        ...
        if (Valida_Inscripcion()) {
            $.ajax({
                type:"POST",
                url:url,
                data:{'nombres':nombres,'apellidos':apellidos,'tel':tel,'mail':mail,'id_conversatorio':id_conversatorio},
                success:function (data) {
                    var val=data.trim();
                    if(val=="validez"){
                        Swal({
