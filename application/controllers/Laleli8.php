...
class Laleli8 extends CI_Controller {
    public function __construct() {
        parent::__construct();
        $this->load->model('Model_Laleli8');
    ...
    public function index(){
        if ($this->session->userdata('usuario')) {
            $dato['fondo'] = $this->Model_Laleli8->fondo_index();
            ...
            $dato['list_nav_sede'] = $this->Model_Laleli8->get_list_nav_sede();
            $dato['valida_punto_venta'] = $this->Model_Laleli8->get_punto_venta();

            $this->load->view('view_LA8/administrador/index',$dato);
        }else{
            redirect('/login');
        }
    }    
    ...
    public function Detalle_Aviso() {
        if ($this->session->userdata('usuario')) { 
            ...
            $this->load->view('view_LA8/aviso/detalle',$dato);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Almacen(){
        if ($this->session->userdata('usuario')) {
            ...
            $this->load->view('view_LA8/almacen/index',$dato);
        }else{
            redirect('/login'); 
        }
    }
    ...
    public function Lista_Almacen() {
        if ($this->session->userdata('usuario')) {
            ...
            $dato['list_almacen'] = $this->Model_Laleli8->get_list_almacen($tipo);
            $this->load->view('view_LA8/almacen/lista',$dato);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Excel_Almacen($tipo){
        $list_almacen = $this->Model_Laleli8->get_list_almacen($tipo);
    ...
    public function Detalle_Almacen($id_almacen){
        if ($this->session->userdata('usuario')) {
            $dato['get_id'] = $this->Model_Laleli8->get_id_almacen($id_almacen);
            $dato['list_anio'] = $this->Model_Laleli8->get_list_anio();
            $dato['list_combo_empresa'] = $this->Model_Laleli8->get_list_empresa();
            $dato['list_sede'] = $this->Model_Laleli8->get_list_sede();
            $dato['list_usuario'] = $this->Model_Laleli8->get_list_usuario();
            $dato['list_estado'] = $this->Model_Laleli8->get_list_estado();
            ...
            $this->load->view('view_LA8/almacen/detalle',$dato);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Lista_Detalle_Almacen(){
        if ($this->session->userdata('usuario')) {
            ...
            $dato['list_detalle_almacen'] = $this->Model_Laleli8->get_list_detalle_almacen($id_almacen);
            $this->load->view('view_LA8/almacen/lista_detalle',$dato);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Detalle_Transferir_Producto($id_almacen){
        if ($this->session->userdata('usuario')) { 
            $dato['get_id'] = $this->Model_Laleli8->get_id_almacen($id_almacen);
            $dato['list_almacen'] = $this->Model_Laleli8->get_list_almacen_combo_transferencia($id_almacen,$dato['get_id'][0]['principal'],$dato['get_id'][0]['id_empresa'],$dato['get_id'][0]['id_vendedor']);
            ...
            $this->load->view('view_LA8/almacen/transferir_producto',$dato);   
        }else{
            redirect('/login');
        }
    }
    ...
    public function Buscar_Producto_Transferir(){
        if ($this->session->userdata('usuario')) {
            ...
            $dato['list_producto'] = $this->Model_Laleli8->get_list_producto_transferir($dato['almacen_actual']);
            $this->load->view('view_LA8/almacen/lista_temporal_t',$dato);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Detalle_Transferencia(){ 
        if ($this->session->userdata('usuario')) {
            ...
            foreach($_POST['productos'] as $list){
                ...
                $get_id = $this->Model_Laleli8->get_cod_producto_transferir($dato['almacen_actual'],$list);
            ...
            $this->load->view('view_LA8/almacen/lista_detalle_t',$dato);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Valida_Transferir_Producto(){
        if ($this->session->userdata('usuario')) {
            ...
            foreach($_POST['productos'] as $list){
                ...
                $valida = $this->Model_Laleli8->valida_stock_transferir_producto($dato);
            ...
            if($c_error>0){
                ...
                foreach($_POST['productos'] as $list){
                    ...
                    if($dato['transferido']>0){
                        $get_id = $this->Model_Laleli8->get_id_almacen($dato['id_almacen']);
                        $list_producto = $this->Model_Laleli8->get_list_producto_disponible_transferir($get_id[0]['principal'],$get_id[0]['id_empresa']);
                        ...
                        if($busqueda!=false){
                            ...
                            if(count($valida)>0){
                                $get_producto = $this->Model_Laleli8->ultimo_producto_activo($dato['cod_producto']);
                                $dato['id_producto'] = $get_producto[0]['id_producto'];
                                $this->Model_Laleli8->insert_transferir_producto_de($dato);
                                $this->Model_Laleli8->insert_transferir_producto_para($dato);
                            }
                        }
                    }
                }
    ...
    public function Detalle_Retirar_Producto($id_almacen){
        if ($this->session->userdata('usuario')) { 
            ...
            $this->Model_Laleli8->delete_todo_temporal_retirar_producto($dato); 
            ...
            $this->load->view('view_LA8/almacen/retirar_producto',$dato);   
        }else{
            redirect('/login');
        }
    }
    ...
    public function Lista_Temporal_Retirar_Producto(){
        if ($this->session->userdata('usuario')) {
            $id_almacen = $this->input->post("id_almacen");
            $dato['list_temporal'] = $this->Model_Laleli8->get_list_temporal_retirar_producto($id_almacen);
            $this->load->view('view_LA8/almacen/lista_temporal_r',$dato); 
        }else{
            redirect('/login');
        }
    }
    ...
    public function Modal_Retirar_Producto_Almacen($id_almacen){
        if ($this->session->userdata('usuario')) {
            $get_almacen = $this->Model_Laleli8->get_id_almacen($id_almacen);
            $dato['list_tipo_producto'] = $this->Model_Laleli8->get_list_tipo_producto_almacen($get_almacen[0]['id_empresa']);
            $this->load->view('view_LA8/almacen/modal_retirar_producto',$dato);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Traer_Producto_Retirar(){
        if ($this->session->userdata('usuario')) {
            ...
            $id_almacen = $this->input->post("id_almacen");
            $dato['list_producto'] = $this->Model_Laleli8->get_list_producto_tipo($id_tipo_producto,$id_almacen);
            $this->load->view('view_LA8/almacen/traer_retirar_producto',$dato);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Insert_Modal_Retirar_Producto_Almacen(){
        if ($this->session->userdata('usuario')) {
            ...
            $validar = $this->Model_Laleli8->valida_insert_temporal_retirar_producto($dato);

            if(count($validar)>0){
                $dato['ingresado']= $validar[0]['ingresado']+1;
                $this->Model_Laleli8->update_temporal_retirar_producto($dato);
            }else{
                $get_producto = $this->Model_Laleli8->ultimo_producto_activo($dato['cod_producto']);
                $dato['id_producto'] = $get_producto[0]['id_producto'];
                $this->Model_Laleli8->insert_temporal_retirar_producto($dato);
            }
        }else{
            redirect('/login');
        }
    ...
    public function Buscar_Producto_Retirar(){
        if ($this->session->userdata('usuario')) {
            ...
            $get_almacen = $this->Model_Laleli8->get_id_almacen($dato['id_almacen']);
            $principal = $get_almacen[0]['principal'];
            $id_empresa = $get_almacen[0]['id_empresa'];

            $dato['get_id'] = $this->Model_Laleli8->get_cod_producto($cod_producto,$principal,$id_empresa);

            if(count($dato['get_id'])==0){
                echo "error";
            }else{
                $dato['cod_producto'] = $cod_producto;
                $dato['stock'] = $this->Model_Laleli8->get_stock_producto($dato);
                $this->load->view('view_LA8/almacen/datos_producto_r',$dato);
            }
        }else{
            redirect('/login');
        }
    ...
    public function Insert_Temporal_Retirar_Producto(){ 
        if ($this->session->userdata('usuario')) {
            ...
            $stock = $this->Model_Laleli8->get_stock_producto($dato);
    ...
    public function Delete_Temporal_Retirar_Producto(){
        if ($this->session->userdata('usuario')) {
            ...
            $this->Model_Laleli8->delete_temporal_retirar_producto($dato);            
        }else{
            redirect('/login');
        }
    } 
    ...
    public function Valida_Retirar_Producto(){ 
        if ($this->session->userdata('usuario')) {
            $dato['id_almacen']= $this->input->post("id_almacen");
            $list_temporal = $this->Model_Laleli8->get_list_temporal_retirar_producto($dato['id_almacen']);
            ...
            if(count($list_temporal)==0){
                ...
                foreach($list_temporal as $list){
                    ...
                    $validar = $this->Model_Laleli8->valida_stock_retirar_producto($dato);
                ...
                if($c_error>0){
                    ...
                    foreach($list_temporal as $list){
                        ...
                        if(count($validar)>0){
                            $this->Model_Laleli8->insert_retirar_producto($dato);
                        }
                    }
    ...
    public function Excel_Detalle_Almacen($id_almacen){
        $list_detalle = $this->Model_Laleli8->get_list_detalle_almacen($id_almacen);
    ...
    public function Movimiento_Almacen($id_almacen,$cod_producto){
        if ($this->session->userdata('usuario')) {
            ...
            $dato['get_id'] = $this->Model_Laleli8->get_id_almacen($id_almacen);
            $dato['get_producto'] = $this->Model_Laleli8->get_cod_producto_movimiento($cod_producto);
            ...
            $this->load->view('view_LA8/almacen/movimiento',$dato);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Lista_Movimiento_Almacen(){
        if ($this->session->userdata('usuario')) {
            $id_almacen = $this->input->post("id_almacen");
            $cod_producto = $this->input->post("cod_producto");
            $dato['list_movimiento_almacen'] = $this->Model_Laleli8->get_list_movimiento_almacen($id_almacen,$cod_producto);
            $this->load->view('view_LA8/almacen/lista_movimiento',$dato);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Excel_Movimiento_Almacen($id_almacen,$cod_producto){
        $cod_producto = str_replace('_','-',$cod_producto);
        $list_movimiento_almacen = $this->Model_Laleli8->get_list_movimiento_almacen($id_almacen,$cod_producto);
    ...
    public function Nueva_Venta(){
        if ($this->session->userdata('usuario')) {
            $get_almacen = $this->Model_Laleli8->get_punto_venta();
            $valida = $this->Model_Laleli8->valida_insert_nueva_venta($get_almacen[0]['id_almacen']);

            if(count($valida)==0){
                $this->Model_Laleli8->insert_nueva_venta($get_almacen[0]['id_almacen']);
            }else{
                $this->Model_Laleli8->resetear_nueva_venta($get_almacen[0]['id_almacen']);
            }
            ...
            $this->load->view('view_LA8/nueva_venta/index',$dato);
        }else{
            redirect('/login'); 
        }
    }
    ...
    public function Alumno_Nueva_Venta(){
        if ($this->session->userdata('usuario')) {
            $get_almacen = $this->Model_Laleli8->get_punto_venta();
            $dato['list_alumno'] = $this->Model_Laleli8->get_list_alumno_nueva_venta($get_almacen[0]['id_empresa'],0,0);
            $dato['get_id'] = $this->Model_Laleli8->get_list_nueva_venta($get_almacen[0]['id_almacen']);

            $get_id = $this->Model_Laleli8->valida_insert_nueva_venta($get_almacen[0]['id_almacen']);

            if($get_id[0]['id_alumno']>0){
                $dato['valida_alumno'] = 1;
                $dato['get_alumno'] = $this->Model_Laleli8->get_list_alumno_nueva_venta($get_almacen[0]['id_empresa'],$get_id[0]['id_alumno'],$get_id[0]['tipo_alumno']);
            }else{
                $dato['valida_alumno'] = 0;
            }

            $this->load->view('view_LA8/nueva_venta/alumno',$dato);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Lista_Producto_Nueva_Venta(){
        if ($this->session->userdata('usuario')) {
            $get_almacen = $this->Model_Laleli8->get_punto_venta();
            $dato['list_nueva_venta'] = $this->Model_Laleli8->get_list_producto_nueva_venta($get_almacen[0]['id_almacen']);
            $this->load->view('view_LA8/nueva_venta/lista',$dato);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Facturacion_Nueva_Venta(){
        if ($this->session->userdata('usuario')) {
            $dato['get_almacen'] = $this->Model_Laleli8->get_punto_venta();
            $dato['get_id'] = $this->Model_Laleli8->get_list_nueva_venta($dato['get_almacen'][0]['id_almacen']);
            $this->load->view('view_LA8/nueva_venta/facturacion',$dato);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Botones_Nueva_Venta(){
        if ($this->session->userdata('usuario')) {
            $get_almacen = $this->Model_Laleli8->get_punto_venta();
            $list_nueva_venta = $this->Model_Laleli8->get_list_producto_nueva_venta($get_almacen[0]['id_almacen']);
            $subtotal = 0;
            foreach($list_nueva_venta as $list){
                $subtotal = $subtotal+($list['cantidad']*$list['precio']);
            }
            $dato['subtotal'] = $subtotal;
            $this->load->view('view_LA8/nueva_venta/botones',$dato);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Detalle_Nueva_Venta(){
        if ($this->session->userdata('usuario')) {
            $get_almacen = $this->Model_Laleli8->get_punto_venta();
            $dato['get_id'] = $this->Model_Laleli8->get_list_nueva_venta($get_almacen[0]['id_almacen']);
            $this->load->view('view_LA8/nueva_venta/detalle',$dato);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Update_Alumno_Nueva_Venta(){
        if ($this->session->userdata('usuario')) {
            ...
            $this->Model_Laleli8->update_alumno_nueva_venta($dato);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Delete_Alumno_Nueva_Venta(){
        if ($this->session->userdata('usuario')) {
            $get_almacen = $this->Model_Laleli8->get_punto_venta();
            $this->Model_Laleli8->delete_alumno_nueva_venta($get_almacen[0]['id_almacen']);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Modal_Producto_Nueva_Venta(){
        if ($this->session->userdata('usuario')) {
            ...
            $dato['list_tipo_producto'] = $this->Model_Laleli8->get_list_tipo_producto_almacen($get_almacen[0]['id_empresa']);
            $dato['list_nueva_venta'] = $this->Model_Laleli8->get_list_producto_nueva_venta($get_almacen[0]['id_almacen']);
            $this->load->view('view_LA8/nueva_venta/modal_producto',$dato);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Lista_Modal_Nueva_Venta(){
        if ($this->session->userdata('usuario')) {
            $get_almacen = $this->Model_Laleli8->get_punto_venta();
            $dato['list_nueva_venta'] = $this->Model_Laleli8->get_list_producto_nueva_venta($get_almacen[0]['id_almacen']);
            $this->load->view('view_LA8/nueva_venta/lista_modal_nueva_venta',$dato);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Traer_Producto_Nueva_Venta(){
        if ($this->session->userdata('usuario')) {
            $this->load->view('view_LA8/nueva_venta/traer_producto'); 
        }else{
            redirect('/login');
        }
    }
    ...
    public function Volver_Producto_Nueva_Venta(){
        if ($this->session->userdata('usuario')) {
            $get_almacen = $this->Model_Laleli8->get_punto_venta();
            $dato['list_tipo_producto'] = $this->Model_Laleli8->get_list_tipo_producto_almacen($get_almacen[0]['id_empresa']);
            $this->load->view('view_LA8/nueva_venta/volver_producto',$dato); 
        }else{ 
            redirect('/login');
        }
    }
    ...
    public function Lista_Modal_Producto_Nueva_Venta(){
        if ($this->session->userdata('usuario')) {
            ...
            $id_tipo_producto = $this->input->post("id_tipo_producto");
            $dato['list_producto'] = $this->Model_Laleli8->get_list_producto_tipo($id_tipo_producto,$id_almacen);
            $this->load->view('view_LA8/nueva_venta/lista_modal_producto',$dato);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Delete_Modal_Producto_Nueva_Venta(){
        if ($this->session->userdata('usuario')) {
            ...
            $dato['id_almacen'] = $get_almacen[0]['id_almacen'];
            $dato['cod_producto'] = $this->input->post("cod_producto");
            $validar = $this->Model_Laleli8->valida_insert_nueva_venta_producto($dato);
            $dato['id_nueva_venta_producto'] = $validar[0]['id_nueva_venta_producto'];
            $dato['cantidad'] = $validar[0]['cantidad']-1;
            $this->Model_Laleli8->delete_modal_producto_nueva_venta($dato);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Modal_Botones_Nueva_Venta(){
        if ($this->session->userdata('usuario')) {
            ...
            $this->load->view('view_LA8/nueva_venta/modal_botones',$dato);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Insert_Modal_Producto_Nueva_Venta(){
        if ($this->session->userdata('usuario')) {
            ...
            $dato['cod_producto']= $this->input->post("cod_producto");
            $get_almacen = $this->Model_Laleli8->get_punto_venta();
            $get_id = $this->Model_Laleli8->get_cod_producto_nueva_venta($get_almacen[0]['id_almacen'],$dato['cod_producto'],$tipo_alumno);
            ...
            if(count($get_id)>0){
                ...
                if(count($validar)>0){
                    ...
                    if($stock[0]['stock']<=0){
                        $this->Model_Laleli8->update_nueva_venta_producto($dato);    
                    }else{
                    ...
                    if($stock[0]['stock']>0){
                        $this->Model_Laleli8->insert_nueva_venta_producto($dato); 
                    }else{
    ...
    public function Insert_Producto_Nueva_Venta(){ 
        if ($this->session->userdata('usuario')) {
            ...
            $cod_producto = $this->input->post("cod_producto");
            $get_almacen = $this->Model_Laleli8->get_punto_venta();
            $get_id = $this->Model_Laleli8->get_cod_producto_nueva_venta($get_almacen[0]['id_almacen'],$cod_producto,$tipo_alumno);
            ...
            if(count($get_id)>0){
                ...
                $get_almacen = $this->Model_Laleli8->get_punto_venta();
                $dato['id_almacen'] = $get_almacen[0]['id_almacen'];
                $valida = $this->Model_Laleli8->valida_insert_nueva_venta_producto($dato);
    ...
    public function Delete_Producto_Nueva_Venta(){
        if ($this->session->userdata('usuario')) {
            ...
            $this->Model_Laleli8->delete_nueva_venta_producto($dato);       
        }else{
            redirect('/login');
        }
    }
    ...
    public function Update_Facturacion_Nueva_Venta(){ 
        if ($this->session->userdata('usuario')) {
            ...
            $this->Model_Laleli8->update_facturacion_nueva_venta($dato);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Modal_Venta(){
        if ($this->session->userdata('usuario')) {
            ...
            $this->load->view('view_LA8/nueva_venta/modal_detalle',$dato);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Insert_Venta(){
        if ($this->session->userdata('usuario')) {
            ...
            if(count($list_nueva_venta)>0){
                ...
                $get_id = $this->Model_Laleli8->get_list_nueva_venta($get_almacen[0]['id_almacen']);
                $get_almacen = $this->Model_Laleli8->get_punto_venta();
    
                $valida_cierre_caja = $this->Model_Laleli8->valida_cierre_caja($_SESSION['usuario'][0]['id_usuario']);
                ...
                if(count($valida_cierre_caja)==0){
                    if($get_id[0]['id_tipo_documento']==3){
                        ...
                        $cantidad = count($this->Model_Laleli8->cantidad_factura());
                        $serie='FA02';
                        ...
                        $dato['get_contabilidad'] = $this->Model_Laleli8->get_list_contabilidad(1);
                        $dato['porcentajeIgv'] = $dato['get_contabilidad'][0]['valor']*100;
            
                        $dato['tipo_api'] = 2;
                        $dato['tipo_doc'] = 1;
                        $api = $this->Model_Laleli8->get_list_api_maestra($dato);
            
                        if(count($api)>0){
                            $get_almacen = $this->Model_Laleli8->get_punto_venta();
                            $list_producto = $this->Model_Laleli8->get_list_producto_nueva_venta($get_almacen[0]['id_almacen']);
                            ...
                            if(!empty($valida['error'])==true){
                                ...
                                if($decodedData['sunatResponse']['success']==true){
                                    ...
                                    $get_almacen = $this->Model_Laleli8->get_punto_venta();
                                    $dato['id_almacen'] = $get_almacen[0]['id_almacen'];
                                    $this->Model_Laleli8->insert_venta($dato);     
                                    $get_id = $this->Model_Laleli8->ultimo_id_venta();
                                    $dato['id_venta'] = $get_id[0]['id_venta'];
                                    $this->Model_Laleli8->insert_venta_detalle($dato);     
        
                                    $list_nueva_venta_producto = $this->Model_Laleli8->get_list_nueva_venta_producto($dato);
                                    ...
                                    foreach($list_nueva_venta_producto as $list){
                                        ...
                                        if($stock[0]['stock']>0){
                                            $this->Model_Laleli8->insert_venta_almacen($dato);
                                        }else{
                                            $this->Model_Laleli8->update_venta_encomienda($dato);
                                            $i = 0;
                                            while($i<$list['cantidad']){
                                                $this->Model_Laleli8->insert_encomienda($dato);
                                                $i++;
                                            }
                                        }
                                    }            
        
                                    $this->Model_Laleli8->delete_nueva_venta($dato);
                        ...
                        $anioi=date('Y');
                        $anio=substr($anioi, 2,2);
                        $cantidad = count($this->Model_Laleli8->cantidad_boleta());
                        $serie='BA02';
                        ...
                        $cantidad_recibo = $this->Model_Laleli8->cantidad_recibo();
    ...
    public function Entrega_Venta(){
        if ($this->session->userdata('usuario')) {
            ...
            $this->load->view('view_LA8/entrega_venta/index',$dato);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Lista_Entrega_Venta() {
        if ($this->session->userdata('usuario')) {
            ...
            $dato['list_entrega_venta'] = $this->Model_Laleli8->get_list_entrega_venta($dato['tipo']);
            $this->load->view('view_LA8/entrega_venta/lista',$dato);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Update_Venta_Lista(){
        if ($this->session->userdata('usuario')) {
            ...
            $this->Model_Laleli8->update_venta_lista($dato);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Modal_Update_Venta_Entregada($id_venta){
        if ($this->session->userdata('usuario')) {
            $dato['get_id'] = $this->Model_Laleli8->get_list_venta($id_venta);
            $this->load->view('view_LA8/entrega_venta/modal_entrega',$dato);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Traer_Tipo_Envio(){
        if ($this->session->userdata('usuario')) {
            ...
            $this->load->view('view_LA8/entrega_venta/tipo_envio',$dato);   
        }else{
            redirect('/login');
        }
    }
    ...
    public function Enviar_Codigo_Verificacion(){
        if ($this->session->userdata('usuario')) {
            ...
            $this->Model_Laleli8->update_venta_codigo_verificacion($dato);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Update_Venta_Entregada(){
        if ($this->session->userdata('usuario')) {
            ...
            $valida = $this->Model_Laleli8->get_list_venta($dato['id_venta']);

            if($valida[0]['codigo_verificacion']==$dato['codigo_verificacion']){
                $this->Model_Laleli8->update_venta_entregada($dato);
            }else{
                echo "error";
            }
        }else{
    ...
    public function Excel_Entrega_Venta($tipo){
        $list_entrega_venta = $this->Model_Laleli8->get_list_entrega_venta($tipo);
    ...
    public function Encomienda(){
        if ($this->session->userdata('usuario')) {
            ...
            $this->load->view('view_LA8/encomienda/index',$dato); 
        }else{
            redirect('/login'); 
        }
    }
    ...
    public function Lista_Encomienda() {
        if ($this->session->userdata('usuario')) {
            $tipo = $this->input->post("tipo");
            $dato['list_encomienda'] = $this->Model_Laleli8->get_list_encomienda($tipo);
            $this->load->view('view_LA8/encomienda/lista',$dato);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Modal_Entrega_Encomienda($id_encomienda){
        if ($this->session->userdata('usuario')) {
            ...
            $this->load->view('view_LA8/encomienda/modal_entrega',$dato);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Entrega_Encomienda() {
        if ($this->session->userdata('usuario')) {
            ...
            $get_id = $this->Model_Laleli8->get_id_encomienda($dato['id_encomienda']);
            $dato['id_almacen'] = $get_id[0]['id_almacen'];
            $dato['id_producto'] = $get_id[0]['id_producto'];
            $dato['cod_producto'] = $get_id[0]['cod_producto'];

            $valida = $this->Model_Laleli8->get_stock_producto($dato);

            if($valida[0]['stock']>0){
                $dato['id_venta'] = $get_id[0]['id_venta']; 

                $this->Model_Laleli8->entrega_encomienda($dato);

                $valida = $this->Model_Laleli8->valida_encomienda_terminada($dato['id_venta']);
                if(count($valida)==0){
                    $this->Model_Laleli8->update_encomienda_terminada($dato['id_venta']);
                }
            }else{
                echo "error";
            }
    ...
    public function Excel_Encomienda($tipo){ 
        $list_encomienda = $this->Model_Laleli8->get_list_encomienda($tipo);
    ...
    public function Venta(){  
        if ($this->session->userdata('usuario')) {
            ...
            $this->load->view('view_LA8/venta/index',$dato);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Lista_Venta() {
        if ($this->session->userdata('usuario')) {
            $get_almacen = $this->Model_Laleli8->get_punto_venta();
            $dato['list_venta'] = $this->Model_Laleli8->get_list_venta($get_almacen[0]['id_almacen']);
            $this->load->view('view_LA8/venta/lista',$dato);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Modal_Anular_Venta($id_venta){
        if ($this->session->userdata('usuario')) {
            $dato['get_id'] = $this->Model_Laleli8->get_id_venta($id_venta);
            $this->load->view('view_LA8/venta/modal_anular', $dato);    
        }else{
            redirect('/login');
        }
    }
    ...
    public function Anular_Venta(){
        if ($this->session->userdata('usuario')) {
            ...
            $get_id = $this->Model_Laleli8->get_id_venta($dato['id_venta']);
            $dato['id_vendedor'] = $get_id[0]['user_reg'];

            $valida_cierre_caja = $this->Model_Laleli8->valida_cierre_caja($dato['id_vendedor']);

            if(count($valida_cierre_caja)==0){
                $valida = $this->Model_Laleli8->valida_aprobar_devolucion($get_id[0]['user_reg'],$get_id[0]['id_sede']);

                if($valida[0]['total']<$get_id[0]['total']){
                    echo "error";
                }else{
                    $this->Model_Laleli8->insert_aprobar_devolucion($dato);
    ...
    public function Excel_Venta(){
        $get_almacen = $this->Model_Laleli8->get_punto_venta();
        $list_venta = $this->Model_Laleli8->get_list_venta($get_almacen[0]['id_almacen']);
    ...
    public function Pdf_Venta($id_venta){
        $get_id = $this->Model_Laleli8->get_id_venta($id_venta); 
        $list_detalle = $this->Model_Laleli8->get_list_venta_detalle($id_venta);
    ...
    public function Recibo_Venta($id_venta){
        if ($this->session->userdata('usuario')) {
            $dato['get_id'] = $this->Model_Laleli8->get_id_venta($id_venta);
            $dato['list_detalle'] = $this->Model_Laleli8->get_list_venta_detalle($id_venta);
            ...
            $html = $this->load->view('view_LA8/venta/recibo',$dato,true);
            $mpdf->WriteHTML($html);
            $mpdf->Output();
        }else{
            redirect('');
    ...
    public function Cierre_Caja(){
        if ($this->session->userdata('usuario')) {
            ...
            $this->load->view('view_LA8/cierre_caja/index',$dato); 
        }else{
            redirect('/login'); 
        }
    }
    ...
    public function Lista_Cierre_Caja() {
        if ($this->session->userdata('usuario')) {
            $tipo = $this->input->post("tipo");
            $dato['list_cierre_caja'] = $this->Model_Laleli8->get_list_cierre_caja($tipo);
            $this->load->view('view_LA8/cierre_caja/lista',$dato); 
        }else{
            redirect('/login');
        }
    }
    ...
    public function Modal_Cierre_Caja(){
        if ($this->session->userdata('usuario')) { 
            $fecha = date('Y-m-d');
            $dato['vendedores'] = $this->Model_Laleli8->get_punto_venta();
            $dato['list_usuario'] = $this->Model_Laleli8->get_list_usuario_codigo();
            //$dato['list_sup_adm'] = $this->Model_Laleli8->get_list_usuario_supervisor_administrador();
            $dato['list_adm_res'] = $this->Model_Laleli8->get_list_usuario_administrador_responsable(); 
            $dato['get_saldo'] = $this->Model_Laleli8->get_saldo_automatico($_SESSION['usuario'][0]['id_usuario'],$fecha);
            $dato['get_producto'] = $this->Model_Laleli8->get_productos($_SESSION['usuario'][0]['id_usuario'],$fecha);
            $this->load->view('view_LA8/cierre_caja/modal_registrar',$dato);    
        }else{
            redirect('/login');
        }
    }
    ...
    public function Saldo_Fecha(){ 
        if ($this->session->userdata('usuario')) {
            ...
            $dato['get_saldo'] = $this->Model_Laleli8->get_saldo_automatico($id_vendedor,$fecha);
            $this->load->view('view_LA8/cierre_caja/saldo',$dato);    
        }else{
            redirect('/login');
        }
    }
    ...
    public function Productos_Fecha(){ 
        if ($this->session->userdata('usuario')) {
            $id_vendedor = $this->input->post("id_vendedor"); 
            $fecha = $this->input->post("fecha"); 
            $get_producto = $this->Model_Laleli8->get_productos($id_vendedor,$fecha);
    ...
    public function Insert_Cierre_Caja(){   
        if ($this->session->userdata('usuario')) {
            ...
            $validar = $this->Model_Laleli8->valida_insert_cierre_caja($dato); 

            if(count($validar)>0){
                //echo "error";
                $dato['id_cierre_caja'] = $validar[0]['id_cierre_caja'];
                $this->Model_Laleli8->update_cierre_caja($dato);
                echo $dato['id_cierre_caja'];
            }else{
                $dato['fecha_valida']= $this->input->post("fecha_i"); 
                $valida_movimiento = $this->Model_Laleli8->valida_venta_cierre_caja($dato);

                if($valida_movimiento[0]['cantidad']>0){
                    $cantidad_devolucion = $this->Model_Laleli8->valida_devoluciones_cierre_caja($dato);

                    if($cantidad_devolucion[0]['cantidad']>0){
                        echo "devolucion";
                    }else{
                        $dato['fecha_valida'] = date("Y-m-d",strtotime($dato['fecha']."- 1 day"));
                        $cantidad = $this->Model_Laleli8->valida_venta_cierre_caja($dato);
        
                        if($cantidad[0]['cantidad']>0){
                            $validar = $this->Model_Laleli8->valida_ultimo_cierre_caja($dato);
        
                            if(count($validar)>0){ 
                                $this->Model_Laleli8->insert_cierre_caja($dato);
                                $get_id = $this->Model_Laleli8->ultimo_id_cierre_caja(); 
    ...
    public function Modal_Update_Cierre_Caja($id_cierre_caja){ 
        if ($this->session->userdata('usuario')) {  
            $dato['get_id'] = $this->Model_Laleli8->get_id_cierre_caja($id_cierre_caja);
            $this->load->view('view_LA8/cierre_caja/modal_editar',$dato);    
        }else{
            redirect('/login');
        }
    }
    ...
    public function Update_Cierre_Caja(){
        if ($this->session->userdata('usuario')) {
            ...
            $this->Model_Laleli8->update_cofre_cierre_caja($dato);            
        }else{
            redirect('/login');
        }
    }
    ...
    public function Delete_Cierre_Caja(){
        if ($this->session->userdata('usuario')) { 
            $dato['id_cierre_caja']= $this->input->post("id_cierre_caja");
            $this->Model_Laleli8->delete_cierre_caja($dato);            
        }else{
            redirect('/login');
        }
    }
    ...
    public function Excel_Cierre_Caja($tipo){
        $list_cierre_caja = $this->Model_Laleli8->get_list_cierre_caja($tipo);
    ...
    public function Pdf_Cierre_Caja($id_cierre_caja){
        if ($this->session->userdata('usuario')) {
            ...
            $html = $this->load->view('view_LA8/cierre_caja/recibo',$dato,true);
            $mpdf->WriteHTML($html);
            $mpdf->Output();
        }else{
            redirect('');
    ...
    public function Detalle_Cierre_Caja($id_cierre_caja){
        if ($this->session->userdata('usuario')) {
            ...
            $this->load->view('view_LA8/cierre_caja/detalle',$dato); 
        }else{
            redirect('/login'); 
        }
    }
    ...
    public function Lista_Ingreso_Cierre_Caja() {
        if ($this->session->userdata('usuario')) {
            ...
            $get_id = $this->Model_Laleli8->get_id_cierre_caja($id_cierre_caja);
            $dato['list_ingreso'] = $this->Model_Laleli8->get_list_ingreso_cierre_caja($get_id[0]['fecha'],$get_id[0]['id_vendedor']);
            $this->load->view('view_LA8/cierre_caja/lista_ingreso',$dato);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Excel_Ingreso_Cierre_Caja($id_cierre_caja){ 
        $get_id = $this->Model_Laleli8->get_id_cierre_caja($id_cierre_caja);
        $list_ingreso = $this->Model_Laleli8->get_list_ingreso_cierre_caja($get_id[0]['fecha'],$get_id[0]['id_vendedor']);
    ...
    public function Lista_Egreso_Cierre_Caja() {
        if ($this->session->userdata('usuario')) {
            $id_cierre_caja = $this->input->post("id_cierre_caja");
            $get_id = $this->Model_Laleli8->get_id_cierre_caja($id_cierre_caja);
            $dato['list_egreso'] = $this->Model_Laleli8->get_list_egreso_cierre_caja($get_id[0]['fecha'],$get_id[0]['id_vendedor']);
            $this->load->view('view_LA8/cierre_caja/lista_egreso',$dato);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Excel_Egreso_Cierre_Caja($id_cierre_caja){ 
        $get_id = $this->Model_Laleli8->get_id_cierre_caja($id_cierre_caja);
        $list_egreso = $this->Model_Laleli8->get_list_egreso_cierre_caja($get_id[0]['fecha'],$get_id[0]['id_vendedor']);
    ...
    public function Informe_Transferencia(){
        if ($this->session->userdata('usuario')) {
            ...
            $this->load->view('view_LA8/informe_t/index',$dato);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Lista_Informe_Transferencia() { 
        if ($this->session->userdata('usuario')) {
            $dato['list_informe_transferencia'] = $this->Model_Laleli8->get_list_informe_transferencia();
            $this->load->view('view_LA8/informe_t/lista',$dato);
        }else{
            redirect('/login');
        }
    }
    ...
    public function Pdf_Informe_Transferencia(){
        if ($this->session->userdata('usuario')) {
            ...
            $html = $this->load->view('view_LA8/informe_t/pdf',$dato,true);
            $mpdf->WriteHTML($html);
            $mpdf->Output();
        }else{
            redirect('');
    ...
    public function Excel_Informe_Transferencia(){
        $list_informe_transferencia = $this->Model_Laleli8->get_list_informe_transferencia();
