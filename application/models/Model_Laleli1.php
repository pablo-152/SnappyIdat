<?php
class Model_Laleli1 extends CI_Model {
    ...
    function get_list_usuario(){
        $sql = "SELECT id_usuario,CONCAT(usuario_apater,' ',usuario_amater,' ',usuario_nombres) AS nom_usuario,
                usuario_codigo 
                FROM users 
                WHERE estado=2 ORDER BY usuario_apater ASC,usuario_amater ASC,usuario_nombres ASC";
        $query = $this->db->query($sql)->result_Array();
        return $query;
    }
    ...
    function get_list_usuario_supervisor_administrador(){  
        $sql = "SELECT id_supervisor AS id_usuario FROM almacen 
                ...
                WHERE id_sede=12 GROUP BY id_usuario";
        $query = $this->db->query($sql)->result_Array();
        return $query;
    }
    ...
    function get_list_almacen($tipo){
        ...
        $sql = "SELECT al.id_almacen,an.nom_anio,em.cod_empresa,se.cod_sede,al.descripcion,re.usuario_codigo AS nom_responsable,
                ...
                WHERE al.id_sede=12 AND al.estado NOT IN (4) $parte
    ...
    function insert_transferir_producto_para($dato){ 
        ...
        $sql = "INSERT INTO movimiento_almacen (tipo,id_almacen,id_producto,cod_producto,cantidad,
                almacen_movimiento,fec_movimiento,user_movimiento,estado,fec_reg,user_reg)
                VALUES (1,'".$dato['id_almacen']."','".$dato['id_producto']."','".$dato['cod_producto']."',
                '".$dato['transferido']."','".$dato['almacen_actual']."',CURDATE(),$id_usuario,2,
                NOW(),$id_usuario)";
        $this->db->query($sql);
    }
    ...
    function get_list_temporal_retirar_producto($id_almacen){ 
        $id_usuario= $_SESSION['usuario'][0]['id_usuario'];
        $sql = "SELECT td.id_temporal,td.id_producto,td.cod_producto AS codigo,tp.descripcion,td.ingresado
    ...
    function get_punto_venta(){
        $sql = "SELECT id_almacen,id_empresa,id_vendedor,doc_sunat
                FROM almacen
                WHERE id_sede=12 AND estado=2 AND id_vendedor!=''";
        $query = $this->db->query($sql)->result_Array();
        return $query;
    }
    ...
    function get_list_alumno_nueva_venta($id_empresa,$id_alumno,$tipo_alumno){ //$id_alumno=null,$tipo_alumno=null
        $sql = "CALL datos_venta_la ($id_empresa,$id_alumno,$tipo_alumno)";
        /*if(isset($id_alumno) && $id_alumno>0){
            if($tipo_alumno==1){
                if($id_empresa==2){
                    $sql = "SELECT Codigo,
                            CONCAT(Apellido_Paterno,' ',Apellido_Materno) AS Apellidos,Nombre,Grado,Seccion 
                            FROM todos_ll
                            WHERE Id=$id_alumno";
                }else if($id_empresa==3){
                    $sql = "SELECT al.cod_alum AS Codigo,CONCAT(al.alum_apater,' ',al.alum_amater) AS Apellidos,
                            al.alum_nom AS Nombre,gr.nom_grado AS Grado,se.nom_seccion AS Seccion
                            FROM alumno al
                    ...
                    $sql = "SELECT Codigo,CONCAT(Apellido_Paterno,' ',Apellido_Materno) AS Apellidos,Nombre,
                            Grado,Seccion 
                            FROM todos_ls
                            WHERE Id=$id_alumno";
                ...
                $sql = "SELECT codigo_glla AS Codigo,
                        CONCAT(apellido_paterno,' ',apellido_materno) AS Apellidos,nombres AS Nombre,
                        '' AS Grado,'' AS Seccion
                        FROM colaborador 
                        WHERE id_colaborador=$id_alumno";
            }
        }else{
            if($id_empresa==2){
                //$sql = "SELECT Id AS id_alumno,CONCAT(Apellido_Paterno,' ',Apellido_Materno,', ',Nombre,' - (LL1) ',Codigo) AS nom_alumno 
                //        FROM todos_ll
                //        WHERE Alumno='Matriculado' AND Anio=".date('Y')."
                //        ORDER BY nom_alumno ASC";
                $sql = "SELECT Id AS id_alumno,1 AS tipo_alumno,
                        CONCAT(Apellido_Paterno,' ',Apellido_Materno,', ',Nombre,' - (LL1) ',Codigo) AS nom_alumno 
                        FROM todos_ll 
                        WHERE Alumno='Matriculado' AND Anio=".date('Y')." 
                        UNION 
                        SELECT co.id_colaborador AS id_alumno,2 AS tipo_alumno,
                        CONCAT(co.apellido_paterno,' ',co.apellido_materno,', ',co.nombres,' - ',pe.nom_perfil) AS nom_alumno 
                        FROM colaborador co 
                        LEFT JOIN perfil pe ON pe.id_perfil=co.id_perfil 
                        WHERE co.id_empresa=2 AND co.estado=2 
                        ORDER BY nom_alumno ASC";
            }else if($id_empresa==3){
                //$sql = "SELECT id_alumno,CONCAT(alum_apater,' ',alum_amater,', ',alum_nom,' - (BL1) ',cod_alum) AS nom_alumno 
                //        FROM alumno
                //        WHERE estado=2
                //        ORDER BY nom_alumno ASC";
                $sql = "SELECT id_alumno,1 AS tipo_alumno,
                        CONCAT(alum_apater,' ',alum_amater,', ',alum_nom,' - (BL1) ',cod_alum) AS nom_alumno 
                        FROM alumno
                        WHERE estado=2
                        UNION 
                        SELECT co.id_colaborador AS id_alumno,2 AS tipo_alumno,
                        ...
                        FROM colaborador co 
                        LEFT JOIN perfil pe ON pe.id_perfil=co.id_perfil 
                        WHERE co.id_empresa=3 AND co.estado=2 
                        ORDER BY nom_alumno ASC";
            }else if($id_empresa==4){
                //$sql = "SELECT Id AS id_alumno,CONCAT(Apellido_Paterno,' ',Apellido_Materno,', ',Nombre,' - (LS1) ',Codigo) AS nom_alumno 
                //        FROM todos_ls
                //       WHERE Alumno='Matriculado' AND Anio=".date('Y')."
                //        ORDER BY nom_alumno ASC";
                $sql = "SELECT Id AS id_alumno,1 AS tipo_alumno,
                        ...
                        FROM colaborador co 
                        LEFT JOIN perfil pe ON pe.id_perfil=co.id_perfil 
                        WHERE co.id_empresa=4 AND co.estado=2 
                        ORDER BY nom_alumno ASC";
            }else if($id_empresa==6){
                ...
                //        ORDER BY nom_alumno ASC";
                $sql = "SELECT Id AS id_alumno,1 AS tipo_alumno,
                        CONCAT(Apellido_Paterno,' ',Apellido_Materno,', ',Nombre,' - (FV1) ',Codigo) AS nom_alumno 
                        FROM todos_l20
                        WHERE Tipo=1
                        UNION 
                        SELECT co.id_colaborador AS id_alumno,2 AS tipo_alumno,
                        ...
                        FROM colaborador co 
                        LEFT JOIN perfil pe ON pe.id_perfil=co.id_perfil 
                        WHERE co.id_empresa=6 AND co.estado=2 
                        ORDER BY nom_alumno ASC";
            }
        }*/
        $query = $this->db->query($sql)->result_Array();
    ...
    function get_cod_producto_nueva_venta($id_almacen,$cod_producto,$tipo_alumno){ //$id_almacen,$cod_producto,$perfil  
        $parte = "";
        /*if($perfil!=""){
            $parte = "AND vm.nom_subtipo='$perfil'";
        }*/
        if($tipo_alumno==2){
        ...
        $sql = "SELECT vm.codigo,vm.nom_tipo,vm.descripcion,vm.talla,pr.id_producto,pr.precio_venta,
                ...
                WHERE vm.id_almacen=$id_almacen AND vm.codigo='$cod_producto' AND pr.estado=2";
        $query = $this->db->query($sql)->result_Array();
        return $query;
    }
    ...
    function insert_nueva_venta_producto($dato){
        $id_usuario= $_SESSION['usuario'][0]['id_usuario'];
        $sql = "INSERT INTO nueva_venta_producto (id_usuario,id_almacen,id_producto,cod_producto,precio,cantidad) 
                VALUES ($id_usuario,'".$dato['id_almacen']."','".$dato['id_producto']."','".$dato['cod_producto']."',
                '".$dato['precio']."',1)";
        $this->db->query($sql);
    }
    ...
    function insert_venta_almacen($dato){
        ...
        $sql = "INSERT INTO movimiento_almacen (tipo,id_venta,id_almacen,id_producto,cod_producto,cantidad,fec_movimiento,
                user_movimiento,estado,fec_reg,user_reg)
                VALUES (4,'".$dato['id_venta']."','".$dato['id_almacen']."','".$dato['id_producto']."','".$dato['cod_producto']."',
                '".$dato['cantidad']."',CURDATE(),$id_usuario,2,NOW(),$id_usuario)";
        $this->db->query($sql);
    }
    ...
    function get_list_encomienda($tipo){   
        ...
        $sql = "SELECT en.id_encomienda,ve.id_venta,ve.cod_venta,pr.cod_producto,
                ...
                WHERE al.id_vendedor!='' AND al.id_sede=12 AND en.cod_producto=vm.codigo) AS stock,
                ...
                WHERE al.id_sede=12 AND en.estado=2 $parte
    ...
    function get_list_cierre_caja($tipo){  
        ...
        $sql = "SELECT ci.id_cierre_caja,ci.fecha,se.cod_sede,um.usuario_codigo AS cod_vendedor,
                ...
                WHERE ci.id_sede=12 AND ci.estado NOT IN (4) $parte
    ...
    function get_saldo_automatico($id_vendedor,$fecha){ 
        $sql = "SELECT IFNULL((SELECT SUM(vd.precio*vd.cantidad) FROM venta_detalle vd
                LEFT JOIN venta ve ON ve.id_venta=vd.id_venta
                LEFT JOIN almacen al ON al.id_almacen=ve.id_almacen
                WHERE ve.user_reg=$id_vendedor AND DATE(ve.fec_reg)='$fecha' AND ve.estado=2 AND ve.estado_venta=1 AND al.id_sede=12),0) AS venta_diaria,
                IFNULL((SELECT SUM(vd.precio*vd.cantidad) FROM venta_detalle vd
                LEFT JOIN venta ve ON ve.id_venta=vd.id_venta
                LEFT JOIN almacen al ON al.id_almacen=ve.id_almacen
                WHERE ve.user_reg=$id_vendedor AND DATE(ve.fec_reg)='$fecha' AND ve.estado=2 AND ve.estado_venta=3 AND al.id_sede=12),0) AS devolucion_diaria,
                (IFNULL((SELECT SUM(vd.precio*vd.cantidad) FROM venta_detalle vd
                LEFT JOIN venta ve ON ve.id_venta=vd.id_venta
                LEFT JOIN almacen al ON al.id_almacen=ve.id_almacen 
                WHERE ve.user_reg=$id_vendedor AND DATE(ve.fec_reg)='$fecha' AND ve.estado=2 AND ve.estado_venta=1 AND al.id_sede=12),0)-
                IFNULL((SELECT SUM(vd.precio*vd.cantidad) FROM venta_detalle vd
                LEFT JOIN venta ve ON ve.id_venta=vd.id_venta 
                LEFT JOIN almacen al ON al.id_almacen=ve.id_almacen
                WHERE ve.user_reg=$id_vendedor AND DATE(ve.fec_reg)='$fecha' AND ve.estado=2 AND ve.estado_venta=3 AND al.id_sede=12),0)) AS saldo_automatico";
        $query = $this->db->query($sql)->result_Array();
        return $query;
    }
    ...
    function get_productos($id_vendedor,$fecha){ 
        $sql = "SELECT IFNULL((SELECT SUM(vd.cantidad) FROM venta_detalle vd
                ...
                LEFT JOIN venta ve ON ve.id_venta=vd.id_venta 
                LEFT JOIN almacen al ON al.id_almacen=ve.id_almacen
                WHERE ve.user_reg=$id_vendedor AND DATE(ve.fec_reg)='$fecha' AND ve.estado=2 AND ve.estado_venta=3 AND al.id_sede=12),0) AS productos";
        $query = $this->db->query($sql)->result_Array();
        return $query;
    }
    ...
    function valida_insert_cierre_caja($dato){ 
        $sql = "SELECT id_cierre_caja FROM cierre_caja 
                WHERE id_sede=12 AND id_vendedor='".$dato['id_vendedor']."' AND 
    ...
    function valida_venta_cierre_caja($dato){ 
        $sql = "SELECT COUNT(1) AS cantidad FROM venta ve
                ...
                DATE(ve.fec_reg)='".$dato['fecha_valida']."' AND al.id_sede=12";
        $query = $this->db->query($sql)->result_Array();
        return $query;
    }
    ...
    function valida_devoluciones_cierre_caja($dato){ 
        $sql = "SELECT COUNT(1) AS cantidad FROM devolucion de
                ...
                de.estado_d=0 AND al.id_sede=12";
        $query = $this->db->query($sql)->result_Array();
        return $query;
    }
    ...
    function insert_cierre_caja($dato){
        $id_usuario= $_SESSION['usuario'][0]['id_usuario'];
        $sql = "INSERT INTO cierre_caja (id_sede,id_vendedor,fecha,saldo_automatico,
                monto_entregado,id_entrega,cofre,productos,cerrada,estado,fec_reg,user_reg) 
                VALUES (12,'".$dato['id_vendedor']."','".$dato['fecha']."',
                '".$dato['saldo_automatico']."','".$dato['monto_entregado']."',
                '".$dato['id_entrega']."','".$dato['cofre']."','".$dato['productos']."',1,2,
                NOW(),$id_usuario)";
        $this->db->query($sql);
    }
    ...
    function update_cierre_caja($dato){ 
        $id_usuario= $_SESSION['usuario'][0]['id_usuario'];
        $sql = "UPDATE cierre_caja SET monto_entregado='".$dato['monto_entregado']."',
                id_entrega='".$dato['id_entrega']."',cofre='".$dato['cofre']."',
                cerrada=1,fec_reg=NOW(),user_reg=$id_usuario,
    ...
    function delete_cierre_caja($dato){
        $id_usuario= $_SESSION['usuario'][0]['id_usuario']; 
        $sql = "UPDATE cierre_caja SET estado=4,fec_eli=NOW(),
                user_eli=$id_usuario 
                WHERE id_cierre_caja='".$dato['id_cierre_caja']."'";
        $this->db->query($sql);
    }
    ...
    function valida_cierre_caja($id_vendedor){
        $sql = "SELECT id_cierre_caja FROM cierre_caja 
                WHERE id_sede=12 AND id_vendedor=$id_vendedor AND fecha=CURDATE() AND estado=2";
        $query = $this->db->query($sql)->result_Array();
        return $query;
    }
    ...
    function get_id_cierre_caja($id_cierre_caja){  
        $sql = "SELECT ci.*,DATE_FORMAT(ci.fecha,'%d-%m-%Y') AS caja,DATE_FORMAT(ci.fec_reg,'%d-%m-%Y %H:%i') AS fecha_cierre,
                ...
                ve.user_reg=ci.id_vendedor AND al.id_sede=12),0) AS recibos,
                IFNULL((SELECT SUM(vd.cantidad*vd.precio) FROM venta_detalle vd 
                LEFT JOIN venta ve ON ve.id_venta=vd.id_venta
                LEFT JOIN almacen al ON al.id_almacen=ve.id_almacen
                WHERE ve.estado=2 AND ve.estado_venta=1 AND ve.id_tipo_documento=1 AND DATE(ve.fec_reg)=ci.fecha AND 
                ve.user_reg=ci.id_vendedor AND al.id_sede=12),0) AS total_recibos,
                IFNULL((SELECT COUNT(1) FROM venta_detalle vd 
                LEFT JOIN venta ve ON ve.id_venta=vd.id_venta
                LEFT JOIN almacen al ON al.id_almacen=ve.id_almacen
                WHERE ve.estado=2 AND ve.estado_venta=1 AND ve.id_tipo_documento=2 AND DATE(ve.fec_reg)=ci.fecha AND 
                ve.user_reg=ci.id_vendedor AND al.id_sede=12),0) AS boletas,
                IFNULL((SELECT SUM(vd.cantidad*vd.precio) FROM venta_detalle vd 
                LEFT JOIN venta ve ON ve.id_venta=vd.id_venta
                LEFT JOIN almacen al ON al.id_almacen=ve.id_almacen
                WHERE ve.estado=2 AND ve.estado_venta=1 AND ve.id_tipo_documento=2 AND DATE(ve.fec_reg)=ci.fecha AND 
                ve.user_reg=ci.id_vendedor AND al.id_sede=12),0) AS total_boletas,
                IFNULL((SELECT COUNT(1) FROM venta_detalle vd 
                LEFT JOIN venta ve ON ve.id_venta=vd.id_venta
                LEFT JOIN almacen al ON al.id_almacen=ve.id_almacen
                WHERE ve.estado=2 AND ve.estado_venta=1 AND ve.id_tipo_documento=3 AND DATE(ve.fec_reg)=ci.fecha AND 
                ve.user_reg=ci.id_vendedor AND al.id_sede=12),0) AS facturas,
                IFNULL((SELECT SUM(vd.cantidad*vd.precio) FROM venta_detalle vd 
                LEFT JOIN venta ve ON ve.id_venta=vd.id_venta
                LEFT JOIN almacen al ON al.id_almacen=ve.id_almacen
                WHERE ve.estado=2 AND ve.estado_venta=1 AND ve.id_tipo_documento=3 AND DATE(ve.fec_reg)=ci.fecha AND 
                ve.user_reg=ci.id_vendedor AND al.id_sede=12),0) AS total_facturas,
                IFNULL((SELECT COUNT(1) FROM venta_detalle vd 
                LEFT JOIN venta ve ON ve.id_venta=vd.id_venta
                LEFT JOIN almacen al ON al.id_almacen=ve.id_almacen
                WHERE ve.estado=2 AND ve.estado_venta=3 AND DATE(ve.fec_reg)=ci.fecha AND ve.user_reg=ci.id_vendedor AND 
                al.id_sede=12),0) AS devoluciones,
                IFNULL((SELECT SUM(vd.cantidad*vd.precio) FROM venta_detalle vd 
                LEFT JOIN venta ve ON ve.id_venta=vd.id_venta
                LEFT JOIN almacen al ON al.id_almacen=ve.id_almacen
                WHERE ve.estado=2 AND ve.estado_venta=3 AND DATE(ve.fec_reg)=ci.fecha AND ve.user_reg=ci.id_vendedor AND 
                al.id_sede=12),0) AS total_devoluciones,
    ...
    function get_list_ingreso_cierre_caja($fecha,$id_vendedor){ 
        /*
            $sql = "SELECT CASE WHEN ve.estado_venta=1 THEN 'Venta' ELSE 'Devolución' END AS nom_tipo,ve.cod_venta,
                ...
                LEFT JOIN almacen al ON al.id_almacen=ve.id_almacen
                LEFT JOIN users us ON us.id_usuario=ve.user_reg
                WHERE al.id_sede=12 AND DATE(ve.fec_reg)='$fecha' AND ve.user_reg=$id_vendedor AND 
        ...
        */
        $sql = "SELECT CASE WHEN al.id_empresa=2 THEN tl.Codigo 
                ...
                LEFT JOIN users us ON us.id_usuario=ve.user_reg
                WHERE al.id_sede=12 AND DATE(ve.fec_reg)='$fecha' AND 
    ...
    function get_list_informe_transferencia(){ 
        $sql = "SELECT ad.descripcion AS de,ap.descripcion AS para,ma.cod_producto,ma.cantidad, 
                ...
                LEFT JOIN sede se ON se.id_sede=al.id_sede
                WHERE se.cod_sede='LA1' AND al.estado=2) AND ma.tipo=2 AND ma.estado=2
    ...
    function  get_venta_detalle_xproducto($dato){
        $sql = "SELECT vd.*,ve.id_alumno,(vd.precio) AS monto,ve.cod_venta
                FROM venta_detalle vd
                LEFT JOIN venta ve ON ve.id_venta=vd.id_venta
                WHERE vd.id_venta=".$dato['id_venta']." AND (vd.cod_producto='LS18-00000' or vd.cod_producto='LS19-00000')";
        $query = $this->db->query($sql)->result_Array();
        return $query;
    }
    ...
    /*function insert_fotocheck_gl($dato){
        $id_usuario= $_SESSION['usuario'][0]['id_usuario'];
        $sql = "INSERT INTO fotocheck_gl (Id,Fecha_Pago_Fotocheck,Monto_Pago_Fotocheck,Doc_Pago_Fotocheck,
                esta_fotocheck,fec_reg,user_reg)
                VALUES ('".$dato['id_alumno']."',DATE(NOW()),'".$dato['monto']."','".$dato['cod_venta']."',0,
                NOW(),$id_usuario)";
        $this->db->query($sql);
    ...
    function get_id_empresa_colaborador($dato){
        $sql = "select * from colaborador  where id_colaborador = '".$dato['id_alumno']."'";
        $query = $this->db->query($sql)->result_Array();
        return $query;
    }
