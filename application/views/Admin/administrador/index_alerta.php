...
?>
  ...
  <script>
    $(document).ready(function() {
      var pendiente= <?php echo count($get_pendiente); ?>;
      if(pendiente>0){
        Swal(
            'Cargos Por Confirmar!',
            'Tienes Cargos Pendientes Por Confirmar. <br> Revise su correo!',
            'info'
        ).then(function() { });
      }

      var alerta_cargo = <?php echo $alerta_cargo[0]['cantidad']; ?>;
      var alerta_cierre_caja = <?php echo $alerta_cierre_caja[0]['cantidad']; ?>;

      if(alerta_cargo>0){
        Swal({
            title: 'Alerta Roja',
            text: "Tienes Cargo(s) pendiente(s) de aprobar. Aprueba cuanto antes.",
            type: 'error',
      ...
      if(alerta_cierre_caja>0){
        Swal({
            title: 'Alerta Roja',
            text: "Tienes Caja(s) pendiente(s) de cerrar. Cerrar lo antes posible.",
            type: 'error',
            showCancelButton: false,
            confirmButtonColor: '#3085d6',
            confirmButtonText: 'OK',
