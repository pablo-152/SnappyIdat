                                ...
                                while($i<=$repetidor){ ?>
                                    ...
                                    >
                                        <?php
                                            ...
                                            $query_tp=mysqli_query($con, "SELECT pr.s_redes FROM proyecto pr
                                            WHERE pr.id_empresa=$id_empresa AND pr.fec_agenda='$dia' AND 
                                            pr.id_subtipo=$id_subtipop AND pr.status NOT IN (8,9)");
                                            $row_tp=mysqli_fetch_array($query_tp);
                                            $totalRows_tp = mysqli_num_rows($query_tp);

                                            $query_tp1=mysqli_query($con, "SELECT pr.s_redes FROM proyecto pr
                                            WHERE pr.id_empresa=$id_empresa AND pr.fec_agenda='$dia' AND 
                                            pr.id_subtipo=$id_subtipop AND pr.status IN (5,6,7)");
