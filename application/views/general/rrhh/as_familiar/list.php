<table class="table table-hover table-bordered table-striped" id="as_familiar_rrhh" width="100%">
    <thead>
    ...
    <tbody>
        <?php foreach($list as $item) {  ?>                                           
            <tr class="even pointer text-center">    
                ...
                <td>
                    <a title="Editar" data-toggle="modal" data-target="#acceso_modal_mod" app_crear_mod="<?= site_url('General/Modal_Update_AsFamiliar_RRHH') ?>/<?= $item['id']; ?>">
...
<script>
    $(document).ready(function() {
        $('#as_familiar_rrhh thead tr').clone(true).appendTo( '#as_familiar_rrhh thead' );
        $('#as_familiar_rrhh thead tr:eq(1) th').each( function (i) {
            var title = $(this).text();
        ...
        var table=$('#as_familiar_rrhh').DataTable( {
            order: [[0,"asc"]],
            orderCellsTop: true,
            fixedHeader: true,
            pageLength: 25,
