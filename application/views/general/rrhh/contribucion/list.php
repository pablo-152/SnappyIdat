<table class="table table-hover table-bordered table-striped" id="contribucion_rrhh" width="100%">
    <thead>
        <tr style="background-color: #E5E5E5;">
            ...
            <th class="text-center" width="10%">Tipo Contribución</th>
            <th class="text-center" width="10%">Nombre</th>
            <th class="text-center" width="10%">Monto</th>
            <th class="text-center" width="6%" title="Tipo Descuento">Tipo D.</th>
    ...
    <tbody>
        <?php foreach($list as $item) {  ?>                                           
            <tr class="even pointer text-center">     
                ...
                <td class="text-left"><?php echo $item['tipo_contribucion_texto']; ?></td>
                ...
                <td>
                    <a title="Editar" data-toggle="modal" data-target="#acceso_modal_mod" app_crear_mod="<?= site_url('General/Modal_Update_Contribucion_RRHH') ?>/<?= $item['id']; ?>">
...
<script>
    $(document).ready(function() {
        $('#contribucion_rrhh thead tr').clone(true).appendTo( '#contribucion_rrhh thead' );
        $('#contribucion_rrhh thead tr:eq(1) th').each( function (i) {
            var title = $(this).text();
        ...
        var table=$('#contribucion_rrhh').DataTable( {
            order: [[0,"asc"]],
            orderCellsTop: true,
            fixedHeader: true,
            pageLength: 25,
