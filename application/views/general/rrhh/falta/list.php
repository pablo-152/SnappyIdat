<table class="table table-hover table-bordered table-striped" id="falta_rrhh" width="100%">
    <thead>
    ...
    <tbody>
        <?php foreach($list as $item) {  ?>                                           
            <tr class="even pointer text-center">     
                ...
                <td>
                    <a title="Editar" data-toggle="modal" data-target="#acceso_modal_mod" app_crear_mod="<?= site_url('General/Modal_Update_Falta_RRHH') ?>/<?= $item['id']; ?>">
...
<script>
    $(document).ready(function() {
        $('#falta_rrhh thead tr').clone(true).appendTo( '#falta_rrhh thead' );
        $('#falta_rrhh thead tr:eq(1) th').each( function (i) {
            var title = $(this).text();
        ...
        var table=$('#falta_rrhh').DataTable( {
            order: [[0,"asc"]],
            orderCellsTop: true,
            fixedHeader: true,
            pageLength: 25,
