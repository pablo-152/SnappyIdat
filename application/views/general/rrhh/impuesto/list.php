<table class="table table-hover table-bordered table-striped" id="impuesto_rrhh" width="100%">
    <thead>
        <tr style="background-color: #E5E5E5;">
            ...
            <th class="text-center" width="10%">De</th>
            <th class="text-center" width="10%">A</th>
            <th class="text-center" width="15%">Porcentaje de Impuesto</th>
    ...
    <tbody>
        <?php foreach($list as $item) {  ?>                                           
            <tr class="even pointer text-center">       
                ...
                <td class="text-right"><?php echo $item['de']; ?></td>   
                <td class="text-right"><?php echo $item['a']; ?></td> 
                <td><?php echo $item['porcentaje_impuesto']; ?></td> 
                <td><span class="badge" style="background-color:<?php echo $item['color']; ?>;"><?php echo $item['nom_status']; ?></span></td>                                                 
                <td>
                    <a title="Editar" data-toggle="modal" data-target="#acceso_modal_mod" app_crear_mod="<?= site_url('General/Modal_Update_Impuesto_RRHH') ?>/<?= $item['id']; ?>">
...
<script>
    $(document).ready(function() {
        $('#impuesto_rrhh thead tr').clone(true).appendTo( '#impuesto_rrhh thead' );
        $('#impuesto_rrhh thead tr:eq(1) th').each( function (i) {
            var title = $(this).text();
        ...
        var table=$('#impuesto_rrhh').DataTable( {
            order: [[0,"asc"]],
            orderCellsTop: true,
            fixedHeader: true,
            pageLength: 25,
