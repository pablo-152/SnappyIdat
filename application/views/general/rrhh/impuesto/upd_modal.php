<form id="formulario_update" method="POST" enctype="multipart/form-data" class="formulario">
    <div class="modal-header">
        ...
        <h5 class="modal-title"><b>Impuesto (Editar) </b></h5>
    </div>
    ...
    <div class="modal-body" style="max-height:520px; overflow:auto;">
            ...
            <div class="form-group col-lg-4">
                <select name="id_empresae" id="id_empresae" class="form-control" onchange="Traer_Sede_Rrhhe();">
                    ...
                    <?php foreach($list_empresa as $list){ ?>
                        ...
                        <?php if($list['id_empresa']==$impuesto[0]['id_empresa']){ echo "selected"; } ?>>
                            <?= $list['cod_empresa']; ?>
                        </option>
                    <?php } ?>
                </select>
            ...
            <div class="form-group col-lg-4">
                <select name="id_sedee" id="id_sedee" class="form-control" onchange="Traer_Tipo_Contrato_Rrhhe();">
                    ...
                    <?php foreach($list_sede as $list){ ?>
                        ...
                        <?php if($list['id_sede']==$impuesto[0]['id_sede']){ echo "selected"; } ?>>
                            <?= $list['cod_sede']; ?>
                        </option>
                    <?php } ?>
                </select>
        ...
        <div class="row">
            ...
            <div class="form-group col-lg-4">
                <select class="form-control" name="id_tipo_contratoe" id="id_tipo_contratoe">
                    ...
                    <?php foreach($list_tipo_contrato as $list){ ?>
                        ...
                        <?php if($list['id']==$impuesto[0]['id_tipo_contrato']){ echo "selected"; } ?>>
                            <?= $list['nombre']; ?>
                        </option>
                    <?php } ?>
                </select>
            ...
            <div class="form-group col-lg-4">
                <input type="text" class="form-control" name="codigoe" id="codigoe" placeholder="Ingresar Código" value="<?= $impuesto[0]['codigo']; ?>">
            </div>
        </div>
        ...
        <div class="row">
            ...
            <div class="form-group col-lg-4">
                <input type="text" class="form-control" name="tipoe" id="tipoe" placeholder="Ingresar Tipo" value="<?= $impuesto[0]['tipo']; ?>">
            </div>
            ...
            <div class="form-group col-lg-4">
                <input type="text" class="form-control" name="subtipoe" id="subtipoe" placeholder="Ingresar Sub-Tipo" value="<?= $impuesto[0]['subtipo']; ?>">
            </div>
        </div>
        ...
        <div class="row">
            ...
            <div class="form-group col-lg-9">
                ...
                pattern="^\d+(\.\d{1,2})?$"
                oninput="this.value = this.value.replace(/[^\d.]/g, '');"
                value="<?= $impuesto[0]['porcentaje_impuesto']?>"
                onblur="if(this.value=='') this.value='0';"
                onfocus="if(this.value=='0') this.value='';"
                class="form-control"
                id="porcentaje_impuesto_e"
                name="porcentaje_impuesto_e"
                placeholder="0"
            ...
            <div class="form-group col-lg-4">
                <select class="form-control" name="id_estado_e" id="id_estado_e">
                    <?php foreach($list_estado as $item){ ?>
                        <option value="<?php echo $item['id_status']; ?>" <?php if($item['id_status']==$impuesto[0]['estado']){ echo "selected"; } ?>>
    ...
    <div class="modal-footer">
        <input type="hidden" name="id_impuesto_e" id="id_impuesto_e" value="<?= $impuesto[0]['id']?>">
        <button type="button" class="btn btn-primary" onclick="Update_Impuesto_RRHH();">
...
<script>
    ...
    $(document).ready(function() {
        activateEnterKeyForFunction(Update_Impuesto_RRHH);
    });

    function Update_Impuesto_RRHH(){
        Cargando();

        var dataString = new FormData(document.getElementById('formulario_update'));
        var url="<?php echo site_url(); ?>General/Update_Impuesto_RRHH";

        if (Valida_Update_Impuesto_RRHH()) {
            $.ajax({
                url: url,
                data:dataString,
                type:"POST",
    ...
    function Valida_Update_Impuesto_RRHH() {
        ...
        if($('#porcentaje_impuesto_e').val().trim() === '') {
            Swal(
                'Ups!',
