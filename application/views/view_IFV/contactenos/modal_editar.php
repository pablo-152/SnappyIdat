<form id="formulario_update" method="POST" enctype="multipart/form-data" class="formulario">
            ...
            <div class="form-group col-md-4">
                <select class="form-control" id="estado_e" name="estado_e" > <!-- onchange="updateInputIdProductValue()" -->
                    ...
                    <?php foreach($list as $item){?> 
                        <option value="<?php echo $item['id_status_general'] ?>" <?php if($get_id[0]['estado']==$item['id_status_general']){echo "selected";}?> ><?php echo $item['nom_status'] ?></option>	
                    <?php }?>
                </select>
            </div>
			
			<input type="hidden" id="id_contactoe" name="id_contactoe" value="<?php echo $get_id[0]['id_contacto']; ?>">
			<!--	
			<div class="form-group col-md-2">
	...
	<div class="modal-footer">		
		<button type="button" class="btn btn-primary" onclick="Actualizar_Contactenos()">
...
<script>
	...
	function Actualizar_Contactenos() {
		$(document)
			.ajaxStart(function() {
				$.blockUI({
					message: '<svg> ... </svg>',
		...
		var url = "<?php echo site_url(); ?>AppIFV/Actualizar_Contactenos";
