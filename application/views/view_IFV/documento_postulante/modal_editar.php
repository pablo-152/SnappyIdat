...
<form id="formulario_update" method="POST" enctype="multipart/form-data" class="formulario">
    <div class="modal-header">
        ...
        <h5 class="modal-title"><b>Editar Documento Postulante</b></h5>
    </div>
    ...
    <div class="modal-body" style="max-height:520px; overflow:auto;">
        <div class="col-md-12 row">
            ...
            <div class="form-group col-md-4">
                <input type="text" class="form-control" id="departamento_u" name="departamento_u" placeholder="Departamento" value="<?php echo $get_id[0]['departamento'] ?>" maxlength="100">
            </div>
            ...
            <div class="form-group col-md-4">
                <select class="form-control" name="id_especialidad_u" id="id_especialidad_u">
                    <option value="0">Todos</option>
                    <?php foreach($list_especialidad as $list){ ?>
                        <option value="<?php echo $list['id_especialidad']; ?>" <?php if($list['id_especialidad']==$get_id[0]['nom_grado']){ echo "selected"; } ?>>
            ...
            <div class="form-group col-md-6">
                <input type="checkbox" class="grande_check" id="aparece_doc_u" name="aparece_doc_u" value="1" <?php if($get_id[0]['aparece_doc']==1){echo "checked";}?>>
                <label class="form-group text-bold label_check">¿Aparece en opción Documentos?</label>
            </div>
        </div>
            ...
            <div class="form-group col-md-4"> 
                <select class="form-control" name="estado_u" id="estado_u">
                    ...
                    <?php foreach($list_status as $list){?>
                        <option value="<?php echo $list['id_confgen']; ?>" <?php if($list['id_confgen']==$get_id[0]['estado']){ echo "selected"; } ?>>
                            <?php echo $list['nom_confgen']; ?>
                        </option> 
                    <?php } ?>
                </select>
            </div>
    ...
    <div class="modal-footer">
        ...
        <button type="button" class="btn btn-primary" onclick="Update_Documento_Postulante()">
...
<script>
    function Update_Documento_Postulante(){
        Cargando();

        var dataString = new FormData(document.getElementById('formulario_update'));
        var url="<?php echo site_url(); ?>AppIFV/Update_Documento_Postulante";

        if (Valida_Update_Documento_Postulante()) {
            $.ajax({
                url: url,
                data:dataString,
                type:"POST",
    ...
    function Valida_Update_Documento_Postulante() {
