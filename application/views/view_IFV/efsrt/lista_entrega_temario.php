<table class="table table-hover table-bordered table-striped" id="example_entrega_temario" width="100%">
    <thead>
    ...
    <tbody>
        <?php foreach($list_entrega_temario as $list){ ?>                                            
            <tr class="even pointer text-center"> 
                ...
                <td>
                    <a title="Reenviar" onclick="Reenviar_Entrega_Temario('<?php echo $list['id_entrega']; ?>','<?= $id_especialidad; ?>')">
                    ...
                    $_SESSION['usuario'][0]['id_usuario']==7 || $_SESSION['usuario'][0]['id_usuario']==85){ ?>
                        <a title="Eliminar" onclick="Delete_Entrega_Temario('<?php echo $list['id_entrega']; ?>')">
...
<script>
    $(document).ready(function() {
        $('#example_entrega_temario thead tr').clone(true).appendTo( '#example_entrega_temario thead' );
        $('#example_entrega_temario thead tr:eq(1) th').each( function (i) {
            var title = $(this).text();
        ...
        var table=$('#example_entrega_temario').DataTable( {
