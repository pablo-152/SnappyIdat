<table class="table table-hover table-bordered table-striped" id="example" width="100%">
    <thead>
        <tr style="background-color: #E5E5E5;">
            ...
            <th class="text-center" width="7%">Valor</th>
            <th class="text-center" width="5%">Ventas</th>
            <th class="text-center" width="7%">Valor</th> 
            <th class="text-center" width="7%">Responsable</th>
            <th class="text-center" width="7%">Supervisor</th>
            <th class="text-center" width="7%">Entrega</th>
            <th class="text-center" width="8%">Administrador</th>
            <th class="text-center" width="4%" title="Vendedor(es)">Vend.</th>
            <th class="text-center" width="4%" title="Principal">Pri.</th>
    ...
    <tbody>
        <?php foreach($list_almacen as $list){ ?>                                           
            <tr class="even pointer text-center" <?php if($list['principal']==1){ ?> style="background-color:#E2F0D9;" <?php } ?>>
                ...
                <td class="text-right"><?php echo "s./ ".$list['v_compra']; ?></td>
                <td><?php echo $list['ventas']; ?></td> 
                <td class="text-right"><?php echo "s./ ".$list['v_venta']; ?></td>
                ...
                <td>
                    <a title="Más Información" href="<?= site_url('Laleli/Detalle_Almacen') ?>/<?php echo $list['id_almacen']; ?>">
                        <img src="<?= base_url() ?>template/img/ver.png"/>
                    </a>

                    <?php if($_SESSION['usuario'][0]['id_usuario']==1 || $_SESSION['usuario'][0]['id_nivel']==6){ ?>
                        <a title="Eliminar" onclick="Delete_Almacen('<?php echo $list['id_almacen']; ?>')">
...
<script>
    $(document).ready(function() {
        ...
        var table = $('#example').DataTable( {
            order: [[13,"desc"],[0,"asc"],[1,"asc"],[3,"asc"]],
            orderCellsTop: true,
            fixedHeader: true,
            pageLength: 50,
            "aoColumnDefs" : [ 
