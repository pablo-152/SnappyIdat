...
<body class="sidebar-xs">
	<div class="navbar navbar-inverse" style="background-color:#65C0E3"> 
		...
		<a class="navbar-brand" href="<?= site_url('Laleli4') ?>"><img src="<?= base_url() ?>template/img/logo2.png" alt=""></a>
				...
				<ul class="nav navbar-nav" style="margin-top: 5px;">
					<select class="form-control" id="nav_cod_sede" name="nav_cod_sede" onchange="Cambiar_Nav_Sede();">
						<?php foreach($list_nav_sede as $list_nav){ ?>
							<option value="<?php echo $list_nav['cod_sede']; ?>" <?php if($list_nav['cod_sede']=="LA4"){ echo "selected"; } ?>>
						...
						<div class="dropdown-menu dropdown-content width-350"> 
							...
							<?php if(count($list_aviso)>5){ ?>
								<div class="dropdown-content-footer">
									<a href="<?= site_url('Laleli4/Detalle_Aviso') ?>" data-popup="tooltip" title="Todos las notificaciones"><i class="icon-menu display-block"></i></a>
								</div>
							<?php } ?>
						</div>
					</li>
					...
					<div class="sidebar-category sidebar-category-visible">
						<div class="category-content no-padding">
							<ul class="navigation navigation-main navigation-accordion" >
								<?php 
								$busq_menu = in_array('ventas_la4', array_column($menu, 'nom_grupomenu'));
								$posicion=array_search('ventas_la4',array_column($menu,'nom_grupomenu'));
								...
								if ($busq_menu != false) { ?>
									<li id="venta">
										...
										<ul id="rventa"> 
											...
											$busq_modulo = in_array('ven_la4_encomiendas', array_column($modulo, 'nom_menu'));
											$posicion = array_search('ven_la4_encomiendas', array_column($modulo, 'nom_menu'));
											if ($busq_modulo != false) { ?>
												<li id="v_encomiendas"><a href="<?= site_url('Laleli4/Encomienda') ?>"><i><svg xmlns="http://www.w3.org/2000/svg"  style="margin: 0 auto;" aria-hidden="true" width="19" height="23" focusable="false" data-prefix="fas" data-icon="sitemap" class="svg-inline--fa fa-sitemap fa-w-20" role="img" viewBox="0 0 640 512"><path fill="currentColor" d="M256 0C273.7 0 288 14.33 288 32V296.6C307.1 307.6 320 328.3 320 352C320 387.3 291.3 416 256 416C220.7 416 192 387.3 192 352C192 328.3 204.9 307.6 224 296.6V32C224 14.33 238.3 0 256 0zM160 128V352C160 405 202.1 448 256 448C309 448 352 405 352 352V128H464C490.5 128 512 149.5 512 176V464C512 490.5 490.5 512 464 512H48C21.49 512 0 490.5 0 464V176C0 149.5 21.49 128 48 128H160z"/></svg></i><?php echo $modulo[$posicion]['nom_subgrupo']; ?></a></li>
											<?php } 

											$busq_modulo = in_array('ven_la4_lista', array_column($modulo, 'nom_menu'));
											$posicion = array_search('ven_la4_lista', array_column($modulo, 'nom_menu'));
											if ($busq_modulo != false) { ?>
												<?php if(count($valida_punto_venta)>0){ ?>
													<li id="v_listas_ventas"><a href="<?= site_url('Laleli4/Venta') ?>"><i class="glyphicon glyphicon-list"></i><?php echo $modulo[$posicion]['nom_subgrupo']; ?></a></li>
												<?php }else{ ?>
								...
								$busq_menu = in_array('almacen_la4', array_column($menu, 'nom_grupomenu')); 
								$posicion=array_search('almacen_la4',array_column($menu,'nom_grupomenu'));?>
								<?php if($busq_menu != false){ ?> 
									<li id="almacen">
										...
										<ul id="ralmacen">
											<?php 
											$busq_modulo = in_array('alm_la4_lista', array_column($modulo, 'nom_menu'));
											$posicion = array_search('alm_la4_lista',array_column($modulo,'nom_menu')); 

											if($busq_modulo != false){?>
												<li id="a_listas_almacenes"><a href="<?= site_url('Laleli4/Almacen') ?>"><i class="glyphicon glyphicon-list" style="font-size: 15px;"></i><?php echo $modulo[$posicion]['nom_subgrupo']; ?></a></li>
											<?php }
											?> 
										</ul>
									</li>
								...
								$busq_menu = in_array('informes_la4', array_column($menu, 'nom_grupomenu'));
								$posicion=array_search('informes_la4',array_column($menu,'nom_grupomenu')); ?>
								...
								<?php if($busq_menu != false){ ?> 
									<li id="informes" class="menu">
										...
										<ul id="rinformes"> 
											<?php 
											$busq_modulo = in_array('inf_la4_transferencia', array_column($modulo, 'nom_menu'));
											$posicion = array_search('inf_la4_transferencia',array_column($modulo,'nom_menu')); 

											if($busq_modulo != false){?>
												<li id="i_transferencias"><a href="<?= site_url('Laleli4/Informe_Transferencia') ?>"><i><svg xmlns="http://www.w3.org/2000/svg"  style="margin: 0 auto;" aria-hidden="true" width="19" height="23" focusable="false" data-prefix="fas" data-icon="sitemap" class="svg-inline--fa fa-sitemap fa-w-20" role="img" viewBox="0 0 640 512"><path fill="currentColor" d="M384 288l64-192h-109.4C308.4 96 281.6 76.66 272 48C262.4 19.33 235.6 0 205.4 0H64l64 288H384zM0 480c0 17.67 14.33 32 32 32h448c17.67 0 32-14.33 32-32v-64H0V480zM480 224h-40.94l-21.33 64H432C440.8 288 448 295.2 448 304S440.8 320 432 320h-352C71.16 320 64 312.8 64 304S71.16 288 80 288h15.22l-14.22-64H32C14.33 224 0 238.3 0 256v128h512V256C512 238.3 497.7 224 480 224z"/></svg></i><?php echo $modulo[$posicion]['nom_subgrupo']; ?></a></li>
											<?php }
											?>
										</ul>
									</li>
