        ...
        <div class="col-md-12 row">
            ...
            <div class="form-group col-md-4">
                <select class="form-control" name="id_vendedor_i" id="id_vendedor_i" onchange="Saldo_Fecha();">
                    ...
                    <?php if($vendedores[0]['id_vendedor']!=""){ ?>
                        <?php $array = explode(",",$vendedores[0]['id_vendedor']); $i = 0; while($i<count($array)){ $posicion = array_search($array[$i],array_column($list_usuario,'id_usuario')); ?>
                            <option value="<?php echo $array[$i]; ?>" <?php if($array[$i]==$id_usuario){ echo "selected"; } ?>>
                                <?php echo $list_usuario[$posicion]['usuario_codigo']; ?>
                            </option> 
                        <?php $i++; } ?>
                    <?php } ?>
                </select>
            ...
            <?php //if($id_usuario==1 || $_SESSION['usuario'][0]['id_nivel']==6 || in_array($id_usuario, $list_sup_adm)){ ?>
                <div class="form-group col-md-4">
                    <input type="date" class="form-control" id="fecha_i" name="fecha_i" value="<?php echo date('Y-m-d'); ?>" onblur="Saldo_Fecha();"> 
                </div>      
            <?php //}else{ ?>
                <!--<div class="form-group col-md-4">
                    <input type="date" class="form-control" id="fecha_i" name="fecha_i" value="<?php echo date('Y-m-d'); ?>" readonly> 
                </div>-->
            <?php //} ?>
        </div>
        ...
        <div class="col-md-12 row">
            ...
            <div class="form-group col-md-4">
                <select class="form-control" name="id_entrega_i" id="id_entrega_i"> 
                    <option value="0">Seleccione</option>
                    <?php foreach($list_adm_res as $list){ ?>
                        <option value="<?php echo $list['id_usuario']; ?>"><?php echo $list['usuario_codigo']; ?></option>
                    <?php } ?>
                </select>
            </div> 

            <?php if($id_usuario==1 || $_SESSION['usuario'][0]['id_nivel']==6){ ?>
                <div class="form-group col-md-2">
...
<script>
    ...
    function Saldo_Fecha(){
        ...
        var url="<?php echo site_url(); ?>Laleli8/Saldo_Fecha"; 
    ...
    function Productos_Fecha(){
        ...
        var url="<?php echo site_url(); ?>Laleli8/Productos_Fecha"; 
        var id_vendedor = $("#id_vendedor_i").val(); 
        var fecha = $("#fecha_i").val(); 
    ...
    function Insert_Cierre_Caja(){
        ...
        var url="<?php echo site_url(); ?>Laleli8/Insert_Cierre_Caja";  
            ...
            $.ajax({
                ...
                success:function (data) {
                    if(data=="error"){
                        ...
                        Swal({
                            title: 'Registro Denegado',
                            text: "¡Tiene solicitudes de devolución pendientes!",
                            type: 'error',
                            showCancelButton: false,
                            confirmButtonColor: '#3085d6',
                            confirmButtonText: 'OK',
                        ...
                        window.open('<?php echo site_url(); ?>Laleli8/Pdf_Cierre_Caja/'+data, '_blank'); 
                        Lista_Cierre_Caja(1);
