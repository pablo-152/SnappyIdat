<!--<table class="table table-hover table-bordered table-striped" id="example_egreso" width="100%">
    <thead>
...
<script>
    $(document).ready(function() {
        $('#example_egreso thead tr').clone(true).appendTo( '#example_egreso thead' );
        $('#example_egreso thead tr:eq(1) th').each( function (i) {
            var title = $(this).text();
        ...
        var table = $('#example_egreso').DataTable( { 
            orderCellsTop: true,
            fixedHeader: true,
            pageLength: 50,
            "aoColumnDefs" : [ 
...
<table class="table table-hover table-bordered table-striped" id="example_egreso" width="100%"> 
    ...
    <tbody>
        <?php foreach($list_egreso as $list){ ?>                                            
            <tr class="even pointer text-center">
