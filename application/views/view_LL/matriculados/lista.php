...
<table id="example" class="table table-hover table-bordered table-striped" width="100%"> 
    <thead>
        <tr style="background-color: #E5E5E5;">
            ...
            <th width="5%" class="text-center">Grado</th>
            <th width="5%" class="text-center">Sección</th>
    ...
    <tbody>
        <?php foreach ($list_alumno as $list) { ?>
            <tr class="even pointer text-center" <?php if($tipo==5){ if($list['id_alumno_retirado']!=''){ ?> style="background-color:#5a485b;" <?php } } ?>>
                ...
                <td class="text-right"><?php echo $list['Monto_Cuota_Ingreso']; ?></td>
                ...
                <td>
                    <?php if($tipo==5){ ?>
                        <a title="Retirar" href="<?= site_url('LittleLeaders/Retiro_Alumno') ?>/<?php echo $list['Id']; ?>">
                            <img src="<?= base_url() ?>template/img/retirar.png">
                        </a>    

                        <?php if($list['id_alumno_retirado']!=""){ ?>
                            <a title="Editar Observación" data-toggle="modal" data-target="#acceso_modal_mod" app_crear_mod="<?= site_url('LittleLeaders/Modal_Update_Obs_Retiro') ?>/<?php echo $list['id_alumno_retirado']; ?>">
                                <img src="<?= base_url() ?>template/img/editar.png">
                            </a>
                        <?php } ?>
                    <?php } ?>

                    <a title="Más Información" href="<?= site_url('LittleLeaders/Detalle_Matriculados') ?>/<?php echo $list['Id']; ?>">
