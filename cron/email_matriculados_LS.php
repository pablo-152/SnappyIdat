<?php 
    ...
    mysqli_query($conexion, "TRUNCATE TABLE nuevos_ls");
    mysqli_query($conexion, "TRUNCATE TABLE apoderados_ls");

    $anio = date('Y');
    $anio_siguiente = date('Y')+1;
    $list_nuevos_ls = sqlsrv_query($conexion_arpay, "SELECT cli.Id,per.FatherSurname AS Apellido_Paterno,per.MotherSurname AS Apellido_Materno,
    ...
    while($fila_alumno = sqlsrv_fetch_array($list_nuevos_ls)){
        mysqli_query($conexion, "INSERT INTO nuevos_ls (Id,Codigo,Apellido_Paterno,Apellido_Materno,Nombre,Dni,
    ...
    $get_Ids_nuevos_ls = mysqli_query($conexion, "SELECT GROUP_CONCAT(DISTINCT Id SEPARATOR ',') AS ids FROM nuevos_ls");
    $get_id = mysqli_fetch_assoc($get_Ids_nuevos_ls); 

    $list_apoderados_ls = sqlsrv_query($conexion_arpay, "SELECT gu.Id,gu.StudentClientId AS Id_Alumno,pe.FatherSurname AS Apellido_Paterno,
    ...
    while($fila_apoderado = sqlsrv_fetch_array($list_apoderados_ls)){
        mysqli_query($conexion, "INSERT INTO apoderados_ls (Id,Id_Alumno,Apellido_Paterno,Apellido_Materno,Nombre,Dni,Email,Celular,
    ...
    while($get_contrato = mysqli_fetch_assoc($query_c)){
        $list_alumno = mysqli_query($conexion, "CALL datos_alumno_contrato (".$fila['id_c_contrato'].",4)");
        ...
        while($fila = mysqli_fetch_assoc($list_alumno)){
            $query_v = mysqli_query($conexion, "SELECT id_documento_firma FROM documento_firma 
                        WHERE id_alumno='".$fila['Id_Alumno']."' AND id_apoderado='".$fila['Id']."' AND id_empresa=4 AND enviado=1 AND 
            ...
            if($total==0){
                ...
                '".$fila['cumpleanos_apoderado']."','".$fila['dni_apoderado']."',4,1,NOW(),
