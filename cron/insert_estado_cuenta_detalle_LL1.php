<?php
    ...
    (
        ...
        WHERE bpi.BankAccountId = 1004 AND i.IsProcessed = convert(bit, 1)  
        ...
        WHERE c.BankAccountId = 1004 AND c.PaymentStatusId IN (1,4)
        ...
        WHERE ap.AccountingPaymentStatusId = 0 AND ap.BankAccountId = 1004
        ...
        SELECT  
            ...
            CASE WHEN st.SourceBankAccountId = 1004 THEN st.Amount * -1 ELSE st.Amount END AS RealAmount,  
            st.OperationNumber AS OperationNumber,  
            st.Observations AS Description,  
            NULL AS Reference  
            FROM SystemMoneyTransfer st  
        WHERE (st.SourceBankAccountId = 1004 OR st.DestinationBankAccountId = 1004)
    ) results  
    ORDER BY  
    ...
    while( $fila = sqlsrv_fetch_array( $query_c, SQLSRV_FETCH_ASSOC) ) {
        ...
        if($total_comercial>0){
            ...
            VALUES ('1004','".$fila['AccountingPaymentId']."','".$fila['SystemMoneyTransferId']."','".$fila['ClientProductPurchaseRegistryId']."','".$fila['MovementType']."','".DATE_FORMAT($fila['Date'], 'Y-m-d H:i:s')."','".DATE_FORMAT($fila['MovementDate'], 'Y-m-d H:i:s')."','".$fila['AmountValue']."','".$fila['RealAmount']."','".$fila['OperationNumber']."','".$fila['Description']."','".$fila['Reference']."',2,NOW(),0)");
        }
    }
