        ...
        while ($fila = mysqli_fetch_assoc($query_c)) { 
            ...
            while ($documento = mysqli_fetch_assoc($query_d)) { 
                if($documento['obligatorio']==3){
                    if($edad<18){
                        sqlsrv_query($conexion_vps, "INSERT INTO detalle_alumno_empresa (id_alumno,id_documento,id_empresa,estado,fec_reg,user_reg) 
                        VALUES (".$fila['Id'].",".$documento['id_documento'].",6,2,getdate(),0)"); 
                    }
                }else{
                    sqlsrv_query($conexion_vps, "INSERT INTO detalle_alumno_empresa (id_alumno,id_documento,id_empresa,estado,fec_reg,user_reg) 
                    VALUES (".$fila['Id'].",".$documento['id_documento'].",6,2,getdate(),0)"); 
