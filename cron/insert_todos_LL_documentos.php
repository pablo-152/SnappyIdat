<?php
    ...
    $query_m = mysqli_query($conexion, "SELECT * FROM todos_ll");
    $total_m = mysqli_num_rows($query_m);
    
    $query_t = mysqli_query($conexion, "SELECT * FROM todos_ll_temporal");
    $total_t = mysqli_num_rows($query_t);

    if($total_t>$total_m){
        $query_c = mysqli_query($conexion, "SELECT Id,Fecha_Cumpleanos FROM todos_ll_temporal 
                ...
                if($documento['obligatorio']==2){
                    if($edad>4){
                        $insert = mysqli_query($conexion, "INSERT INTO detalle_alumno_empresa (id_alumno,id_documento,id_empresa,estado,fec_reg,user_reg) 
                                VALUES (".$fila['Id'].",".$documento['id_documento'].",2,2,NOW(),0)");
                    }
