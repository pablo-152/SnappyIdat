...
class Model_Examen extends CI_Model {
  ...
  function consulta_tiempo_examen($dato){
    $sql = "SELECT * FROM pos_exam_efsrt 
            where idpos_pe='".$dato['codigo']."' and idexa_pe='".$dato['id_examen']."' and fec_examen='".$dato['fec_examen']."' and estado_pe<>1
            ORDER BY id_pe DESC";
    $query = $this->db2->query($sql)->result_Array();
    return $query;
  }

  function get_config_examen()
  {
    $sql = "SELECT * from config_examen_efsrt where estado=2 limit 1";
      $query = $this->db->query($sql)->result_Array();
      return $query;
  }
